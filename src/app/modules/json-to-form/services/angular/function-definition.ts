...
export class FormBuilder {
	...
	private generateMethodFromKeypath(stack: (string | null)[]): MethodFromKeypath {
		...
		const callName = this.hasReservedWord
			? getterName
			: uncapitalized;
		...
		result.getter = {
			...
			withReturn: this.generateFunctionWithReturn('get', getterName, params, path, reactiveFormType),
		};
		...
		result.creator = {
			...
			withReturn: this.generateFunctionWithReturn('create', createFunctionName, params, path, reactiveFormType),
		};

		return result;
	}

	private generateFunctionWithReturn(operation: string, name: string, params: string[], pathArray: string[], reactiveFormType: string): string {
